namespace PriorityQueue
{
        ...
        where T : IComparable<T>
    {
        ...
        public bool IsConsistent()
        {
            ...
            for (int parentIndex = 0; parentIndex < this.heap.Count; ++parentIndex)
            {
                int leftIndex = (2 * parentIndex) + 1;
                int rightIndex = (2 * parentIndex) + 2;

                if (leftIndex <= lastIndex &&
                    this.heap[parentIndex].CompareTo(this.heap[leftIndex]) > 0)
                {
                    return false;
                }

                if (rightIndex <= lastIndex &&
                    this.heap[parentIndex].CompareTo(this.heap[rightIndex]) > 0)
                {
                    return false;
                }
            }
